  ...
  setTimeout(function () {
    ...
    for (var i = 0; i < localStorage.length; i++) {
      txt += `<p class="torles ${localStorage.key(
        ...
        i
      )} fa-sharp fa-solid fa-trash" class="torlesGomb data-key="${localStorage.key(
        i
      )}"></i></p>`;
    }
    txt += `<button type="button" class="rendel ${localStorage.key(
      i
    )}" onclick="rendeles()">Rendel</button>`;
    gomb.append(txt);
  }, 1400);
}
